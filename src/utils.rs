...
pub fn uninstall_old_service() -> Result<(), Error> {
    ...
    let target_binary_path = "/Library/PrivilegedHelperTools/io.github.clashmax.helper";
    let plist_file = "/Library/LaunchDaemons/io.github.clashmax.helper.plist";

    // Stop and unload service
    let _ = run_command("launchctl", &["stop", "io.github.clashmax.helper"], false);
    let _ = run_command("launchctl", &["bootout", "system", plist_file], false);
    let _ = run_command(
        "launchctl",
        &["disable", "system/io.github.clashmax.helper"],
        false,
    )?;
