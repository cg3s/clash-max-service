...
pub fn uninstall_old_service() -> Result<(), Error> {
    ...
    let target_binary_path = "/Library/PrivilegedHelperTools/cg3s.helper";
    let plist_file = "/Library/LaunchDaemons/cg3s.helper.plist";

    // Stop and unload service
    let _ = run_command("launchctl", &["stop", "cg3s.helper"], false);
    let _ = run_command("launchctl", &["bootout", "system", plist_file], false);
    let _ = run_command(
        "launchctl",
        &["disable", "system/cg3s.helper"],
        false,
    )?;
